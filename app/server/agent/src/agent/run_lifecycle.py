...
class AgentRunLifecycleService:
    ...
    def prepare_run_context(
        ...
        if db is not None and (request.platform_id is None or not request.external_user_id or not request.agent_id):
            raise RuntimeError("持久化 Agent 运行缺少 platform_id、external_user_id 或 agent_id")
