...
def create_agent_context_schema() -> type[BaseModel]:
    ...
    return create_model(
        ...
        checkpoint_thread_id=(str, Field(default="", description="内部 Checkpoint 线程 ID")),
        run_id=(str, Field(default="", description="本次 Agent 调用 ID")),
        platform_id=(int | None, Field(default=None, description="业务平台 ID")),
        external_user_id=(str | None, Field(default=None, description="外部业务用户 ID")),
        runtime_credentials=(
            dict[str, str],
            Field(default_factory=dict, exclude=True, repr=False, description="本次运行敏感凭证"),
        ),
